...
var Path = require('path'),
    ...
    deepCrossAdapterRunner = require('../features/deepCrossAdapter/core/deepCrossAdapter'),
    mocha = require('mocha');
...
module.exports = function(options) {
  ...
  var DeepPath = Path.resolve(__dirname,'../features/deepCrossAdapter/core/deepCrossAdapter.js');
  files.push(DeepPath);
  ...
  var runner = test.run(function(err) {
    ...
    if(runCrossAdapterTests){
      ...
      crossAdapterRunner(options, function(crossAdapterErr){
		if (err) {
			crossAdapterErr(err, options.failOnError);
		}
		// run deep
